...
def analyze_color_checker(filename, palette_filename=None, debug_filename=None):
    ...
    for i in range(ROWS):
        for j in range(COLS):
            ...
            patch = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
            corrected_patch = cv2.cvtColor(corrected_patch, cv2.COLOR_BGR2RGB)
            target_patch = cv2.cvtColor(target_patch, cv2.COLOR_BGR2RGB)
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=0.85)  # Adjust for title
    plt.suptitle('Color Checker Patches (left=actual, right=target)', fontsize=16)
    if palette_filename:
        plt.savefig(palette_filename)
    plt.close()
    ...
    for i in range(ROWS):
        for j in range(COLS):
            ...
            corrected_patch = cv2.cvtColor(corrected_patch, cv2.COLOR_BGR2RGB)
            patch = cv2.copyMakeBorder(patch, 0, 0, 0, patch.shape[1], cv2.BORDER_CONSTANT, value=targets[color])
            corrected_patch = cv2.copyMakeBorder(corrected_patch, 0, 0, 0, corrected_patch.shape[1], cv2.BORDER_CONSTANT, value=targets[color])

            # Show the patch
            plt.subplot(ROWS, COLS, i * COLS + j + 1)
            plt.imshow(np.hstack((patch, corrected_patch)))
            plt.title(f'{color}')
            plt.axis('off')
